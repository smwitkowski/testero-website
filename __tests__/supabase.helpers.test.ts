...
describe("supabase server helper", () => {
  ...
  it("silently catches error when cookie set fails", async () => {
    ...
    // Should not throw when cookie set fails
    await expect(opts.cookies.set("a", "b", { path: "/" })).resolves.toBeUndefined();

    // Verify that set was attempted
    expect(store.set).toHaveBeenCalledWith("a", "b", { path: "/" });
  });

  it("silently catches error when cookie remove fails", async () => {
    ...
    // Should not throw when cookie remove fails
    await expect(opts.cookies.remove("a", { path: "/" })).resolves.toBeUndefined();

    // Verify that set was attempted with maxAge: 0
    expect(store.set).toHaveBeenCalledWith("a", "", { path: "/", maxAge: 0 });
  });
});
