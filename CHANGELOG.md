...
### Fixed
- **Early Access Redirect Loop**: Updated route configuration to mark `/early-access-coming-soon` as a public route so that non-early-access users redirected there from protected routes (e.g. `/dashboard`) are not immediately re-checked and redirected again. This prevents infinite 307 redirect loops between protected routes and the early access info page in middleware.
