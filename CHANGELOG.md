...
- **Snapshot Canonical Domain Info into Diagnostic Questions**: Each `diagnostic_questions` row now carries `domain_id` (UUID FK to `exam_domains.id`) and `domain_code` (TEXT matching `exam_domains.code`) as snapshots from canonical questions at session creation time. This enables domain breakdown calculations and future readiness metrics to be computed directly from `diagnostic_questions` without rejoining canonical tables. The diagnostic summary endpoint (`GET /api/diagnostic/summary/[sessionId]`) uses snapshot `domain_code` for canonical PMLE sessions, falling back to legacy `questions.topic` join only for older or non-canonical sessions. Migration `20250120_add_domain_fields_to_diagnostic_questions.sql` adds these nullable fields with indexes for performance. Added unit tests verifying canonical domain breakdown computation without table joins and legacy fallback path preservation.
