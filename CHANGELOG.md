...
### Fixed
- **Auth Confirm Redirect Origin**: Fixed `/auth/confirm` endpoint to use `NEXT_PUBLIC_SITE_URL` (canonical site URL) for all redirects instead of `request.url.origin`. This prevents redirects to `0.0.0.0:3000` in Cloud Run where the internal service origin differs from the public domain. Redirect origin validation now compares against the canonical site URL, ensuring consistent behavior across environments.
