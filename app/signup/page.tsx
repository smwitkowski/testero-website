...
function getSignupErrorCode(message: string): string {
...
function getInputClass(hasError: boolean, isDirty: boolean): string {
  const base =
    'px-4 py-3 text-base rounded-md transition-all duration-300 border-2';
  if (hasError) {
  ...
  return `${base} border-slate-300 focus:border-orange-400 focus:ring focus:ring-orange-200 focus:ring-opacity-50`;
}

function FormFieldIcons({
  hasError,
  isDirty,
}: {
  hasError: boolean;
    ...
    try {
      ...
      if (posthog) {
        posthog.capture('signup_error', {
          error_code: getSignupErrorCode(errorMessage),
        });
      }
    } finally {
                            ...
                            <div className="relative">
                              <FormControl>
                                <Input
                                  ...
                                  className={getInputClass(!!fieldState.error, fieldState.isDirty)}
                              ...
                              <FormFieldIcons hasError={!!fieldState.error} isDirty={fieldState.isDirty} />
                            </div>
