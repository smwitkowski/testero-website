...
import { createBetaVariantAnalyticsProps } from "@/lib/analytics/beta-variant-helpers";
...
export default function BetaWelcomePage() {
  ...
  const handleStartDiagnostic = async () => {
    ...
    try {
      // Track invite click event (start diagnostic is an invite action)
      trackEvent(
        posthog,
        ANALYTICS_EVENTS.INVITE_CLICKED,
        createBetaVariantAnalyticsProps(user, {
          user_id: user.id,
          source: "beta_welcome",
      ...
      trackEvent(
        posthog,
        ANALYTICS_EVENTS.START_DIAGNOSTIC_CLICKED,
        createBetaVariantAnalyticsProps(user, {
          user_id: user.id,
          has_gift_card_incentive: shouldShowGiftCardIncentive(betaVariant),
        })
